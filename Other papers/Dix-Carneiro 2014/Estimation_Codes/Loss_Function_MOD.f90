...
! ***************************************************************************************************************
! This module contains 4 subroutines/functions:
!
...
!                            xsi2005Sim,etaSim,LaborSharesSim,CapitalSharesSim,Loss_Function)!
!
!subroutine Write_Sim_Data(ChoiceSim,WageSim,ExperSim,typ,VSim,Cost1Sim,Cost2Sim,CohortWgt, &
!                          CapitalSim,z,sk_sup_eq,rsk_eq)
! ***************************************************************************************************************

! SMM_OBJ_FCN simulates the economy, estimates auxiliary models over simulated
! data, and computes the Loss Function
Subroutine SMM_OBJ_FCN(ParamScaled,F)
...
rankcond1: if (rank == 0) then        
    ...
    do i = 1, NPARAM
        if (mask_global(i) == 1) then
            if ( (i >= 92 .and. i <= 99) .or. &
                 (i == 132) .or. &
                 (i == 175) .or. (i == 177) .or. (i == 179) .or. (i == 181) .or. &
                 (i == 183) .or. (i == 185) .or. (i == 187) .or. (i == 189) .or. &
                 (i == 191) .or. (i == 193) .or. (i == 195) .or. (i == 197) .or. &
                 (i == 199) .or. (i == 201) .or. &
                 (i >= 203 .and. i <= 223) ) then
                param(i) = exp(ParamScaled(k) * scaling_global(i))   
                k = k + 1         
            else
                param(i) = ParamScaled(k) * scaling_global(i)
                k = k + 1
            end if
        else
            if ( (i >= 92 .and. i <= 99) .or. &
                 ...
                 (i == 199) .or. (i == 201) .or. &
                 (i >= 203 .and. i <= 223) ) then
                param(i) = exp(param_global(i) * scaling_global(i))         
            else
                param(i) = param_global(i) * scaling_global(i)
            end if
        end if
    end do


    open(unit = 9675, file = 'Current_Param.csv')
        do i = 1, NPARAM
            write(9675,*) i, ',', param(i)
        end do
    close(9675)
    ...
    if (FLAG_STOP == 1) then
        ...
        ! *****************************************
        ! Idiosyncractic shocks for the simulation
        ! *****************************************
    ...
    RatExp_loop: do while (RE_loop <= 2)
        ...
        year_loop: do t = FIRST_YR, LAST_YR
            ...
            while_loop: do while (.not. converged .and. it <= MAXIT)
                ...
                ! ParallelCohorts computes the aggregate supply of human capital in parallel for
                ! different cohorts
        ...
        if (rank == 0) then
            ...
            ! Estimating the forecasting rule for HC prices
            dim1 = 9
            dim2 = 2
...
! Save auxiliary models fit to the data vs. fit to simulated data
subroutine WriteCoefToFile(betaSim,SigmaWageDifSim,SigmaWageSim,gammaSim,phiSim,rhoSim,xsi1998Sim,xsi2000Sim,& 
                           xsi2005Sim,etaSim,LaborSharesSim,CapitalSharesSim,Loss_Function)
...
real(KIND=DOUBLE), intent(in) :: betaSim(:,:), & 
                                 ...
                                 xsi2005Sim(0:,:), &
                                 etaSim(0:,:), &
                                 LaborSharesSim(:,0:,FIRST_YR:), &
                                 CapitalSharesSim(:,FIRST_YR:), &
                                 Loss_Function
                                                      

integer s, s1, s2, i, gen, ed, t
...
do s = 1, NSECTORS
    do ed = 0, 1
        do t = FIRST_YR, LAST_YR
            ...
            write(1,40) 'Labor Shares', s, ed, t, LaborSharesData(s,ed,t), LaborSharesSim(s,ed,t)
...
do s = 1, NSECTORS
    do t = FIRST_YR, LAST_YR
        ...
        write(1,39) 'Capital Shares', s, t, CapitalSharesData(s,t), CapitalSharesSim(s,t)
...
40 format(a14, ',' , 3(i5,','), 2(f10.6,','))
39 format(a14, ',' , 2(i5,','), 2(f10.6,','))  

100 format(i4,',',23(f10.6,','))
