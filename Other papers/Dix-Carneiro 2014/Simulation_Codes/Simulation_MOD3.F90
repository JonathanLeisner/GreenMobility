...
real(KIND=DOUBLE) rsk_sup(NSECTORS,0:1,MAXIT), &
                  ...
                  rK_post(NSECTORS,FIRST_YR_SIM:LAST_YR_SIM), &
...
if (rank == 0) then    
    ...
    ! ***************************************************************
    ! This is the last year before the shock in the price of sector 3
    ! ***************************************************************
    ...
    year_loop: do t = FIRST_YR_SIM, LAST_YR_SIM
        ...
        while_loop: do while (.not. converged .and. it <= MAXIT)
            ...
            if (rank == 0) then
                    ...
                    do while (.not. converged2 .and. it2 <= MAXIT)
                        ...
                        do s = 1, NSECTORS-1
                            ...
                            if (s == 1) then
                                ...
                                Call NewtonSolver(Const1,Const2,Const3,sigma_prod(s),init,solution)
                                Cap_dem(s) = solution
                                NewCap(s)  = max(Cap_dem(s) - OldCap(s),0.0)
                                NewCap(s)  = min(0.1*CapitalStock,NewCap(s))
                            else
                                Remaining  = max(0.1*CapitalStock - sum(NewCap(1:s-1)),0.0)
                                Const1 = alpha_prod(s,0,LAST_YR)*sk_sup(s,0)**sigma_prod(s) + alpha_prod(s,1,LAST_YR)*sk_sup(s,1)**sigma_prod(s)
                                Const2 = rK_sup(it2)/z(s,t)
                                Const3 = 1 - alpha_prod(s,0,LAST_YR) - alpha_prod(s,1,LAST_YR)
                                if (t == FIRST_YR_SIM) then
                                ...
                                Cap_dem(s) = solution
                                NewCap(s)  = max(Cap_dem(s) - OldCap(s),0.0)
                                NewCap(s)  = min(Remaining,NewCap(s))
                            end if
                                                                
                        end do
                
                        NewCap(7) = max(0.1*CapitalStock - sum(NewCap(1:6)),0.0)
                ...
                do s = 1, NSECTORS
                    rK_post(s,t) = z(s,t)*( (alpha_prod(s,0,LAST_YR)*sk_sup(s,0)**sigma_prod(s) + alpha_prod(s,1,LAST_YR)*sk_sup(s,1)**sigma_prod(s) + &
                                            (1 - alpha_prod(s,0,LAST_YR) - alpha_prod(s,1,LAST_YR))*Cap(s)**sigma_prod(s))**(1/sigma_prod(s) - 1) )* &
                                            (1 - alpha_prod(s,0,LAST_YR) - alpha_prod(s,1,LAST_YR))*Cap(s)**(sigma_prod(s)-1)
                end do
                ...
                do s = 1, NSECTORS
                    do ed = 0, 1
                        if (sk_sup(s,ed) > 0) then
                            rsk_dem(s,ed,it) = min(( alpha_prod(s,ed,t) / (1 - alpha_prod(s,0,t) - alpha_prod(s,1,t)) )*rK_post(s,t)*((Cap(s)/sk_sup(s,ed))**(1-sigma_prod(s))),1000.0)
                        else
                ...
                if ( .not. converged .and. it < MAXIT ) then
                    ...
                    Welfare(t)       = Welfare(t) + sum(rK_post(:,t)*CapitalSim(:,t))
        ...
        if (rank == 0) then
            ...
            print*
            print*, 'rK:'
            print*, rK_post(1,t), rK_post(2,t), rK_post(3,t), rK_post(4,t), rK_post(5,t), rK_post(6,t), rK_post(7,t)
            print*
            print*, 'rsk Ed=0:'
...
if (rank == 0) then
    ...
    do coh = HALF_YR_SIM - 40 - LAST_AGE, HALF_YR_SIM - 40 - FIRST_AGE
        ...
        open(unit = 1, file = 'AffectedWorkers_Pre3.csv')
    ...
    open(unit = 1, file = 'AffectedWorkers_Post3.csv')
    write(1,4738) 'ID','Cohort','Year','Educ','Choice','Exper','Welfare','Welfare2'
    do ind = 1, Size2 
        coh = coh_index(ind)
        n   = n_index(ind)
    ...
    open(unit = 1, file = 'Simulation3.csv')
        ...
        do t = FIRST_YR_SIM, LAST_YR_SIM
            write(1,3333) t, price(1,t), price(2,t), price(3,t), price(4,t), price(5,t), price(6,t), price(7,t), Pindex(t), &
                          ...
                          sk_sup_eq(1,0,t), sk_sup_eq(2,0,t), sk_sup_eq(3,0,t), sk_sup_eq(4,0,t), sk_sup_eq(5,0,t), sk_sup_eq(6,0,t), sk_sup_eq(7,0,t), &
                          sk_sup_eq(1,1,t), sk_sup_eq(2,1,t), sk_sup_eq(3,1,t), sk_sup_eq(4,1,t), sk_sup_eq(5,1,t), sk_sup_eq(6,1,t), sk_sup_eq(7,1,t), &
                          rK_post(1,t), rK_post(2,t), rK_post(3,t), rK_post(4,t), rK_post(5,t), rK_post(6,t), rK_post(7,t), &
    ...
    open(unit = 1, file = 'AdjCosts3.csv')
    ...
    open(unit = 1, file = 'Reallocation3.csv')
...
end subroutine Simulation3


end module Simulation_MOD3
