libname painel "/home/dix-carneiro/BoostrapSAS/Bootstrap" ;
libname boot "/home/dix-carneiro/BoostrapSAS/Bootstrap" ;
...
%do i = 1 %to 10 ;
	...
	proc surveyselect data = New_Painel out = bootsample
	...
	%do s = 1 %to 7 ;
		...
		data residuals (keep = NumberHits ones idade_new sector resid_2) ;
			set residuals ;
			resid_2 = resid**2 ;
			ones = 1 ;
		run ;
	...
	%do s = 0 %to 7 ;
		...
		model freq&s. = lag_sector0 lag_sector1 lag_sector2 lag_sector3 lag_sector4 lag_sector5 lag_sector6 lag_sector7 
                        gender dummy_educ2 dummy_educ3 dummy_educ4 age age_2 exper1 exper2 
                        exper3 exper4 exper5 exper6 exper7 / noint ;
		weight NumberHits ;
		run ;
		quit ;
...
/*PROC CORR DATA = boot.Bootstrap noprint COV NOCORR OUTP = boot.BOOT_COVAR ;*/
/*	VAR _ALL_ ;*/
/*RUN ;*/
/**/
/*data boot.BOOT_COVAR (drop = _TYPE_ obs _NAME_) ;*/
/*	set boot.BOOT_COVAR ;*/
