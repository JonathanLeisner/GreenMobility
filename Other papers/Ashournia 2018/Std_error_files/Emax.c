...
#define iDelta2     iAge + female * nAge + educ * CatGender * nAge + iLagSec * CatEduc * CatGender * nAge

//-------------------------------------------------------------------------------------
// This function computes the EMAX coefficients when agents have static expectations
// regarding future skill returns.
//-------------------------------------------------------------------------------------
...
void Emax_SE(int tid, double *param, int iFemale, int iEduc, int iElig, int iType, double *Low_RSk, double *Up_RSk, double Low_Wage, double Up_Wage, double *PI_SE, double *R_sq_SE)
{
	...
	int iAge, iLagSec, iSec, iN, iPar, iDraw, iReg, iCnt;
		...
		for (iLagSec = 0; iLagSec < nSector + 1; iLagSec++)
		{
			for (iN = 0; iN < Intp; iN++)
			{
				...
				for (iDraw = 0; iDraw < nDraws; iDraw++)
				{
					...
					for (iSec = 1; iSec < nSector + 1; iSec++)
					{
						V[iSec-1] = (1 - delta[iDelta2])*(w[iSec-1]*exp(eps[iEpsEmax]) + rho*Emax[iSec-1] - Cost[iSec-1]) + delta[iDelta2]*(benefit + rho*Emax0);
...
//-------------------------------------------------------------------------------------
// This function computes the EMAX coefficients when agents have rational expectations
// regarding future skill returns.
//-------------------------------------------------------------------------------------
	         ...
	         double *R_sq_RE)
{
	...
	int iYear, iAge, iDraw, iSec, iLagSec, iN, iPar, iReg, iCnt;
	int female, educ, elig, typ;
	...
	#define iPIRE1		iReg + iYear     * nReg + iAge     * (nYear-1) * nReg + iLagSec * (nAge-1) * (nYear-1) * nReg + iFemale * (nSector+1) * (nAge-1) * (nYear-1) * nReg + iEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) * nReg + iElig * CatEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) * nReg + iType * CatElig * CatEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) * nReg
	#define iPIREEmax	iReg + (iYear-1) * nReg + (iAge-1) * (nYear-1) * nReg + iLagSec * (nAge-1) * (nYear-1) * nReg + iFemale * (nSector+1) * (nAge-1) * (nYear-1) * nReg + iEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) * nReg + iElig * CatEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) * nReg + iType * CatElig * CatEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) * nReg
	#define iRsqRE		(iYear-1) + (iAge-1) * (nYear-1) + iLagSec * (nAge-1) * (nYear-1) + iFemale * (nSector+1) * (nAge-1) * (nYear-1) + iEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) + iElig * CatEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1) + iType * CatElig * CatEduc * CatGender * (nSector+1) * (nAge-1) * (nYear-1)
	...
	//---------------------------------------------------------------------------------
	// Algorithm goes from 2007 to 1997 as workers have static expectations in
	// last year. I subtract 2 from nYear as indexation starts at 0.
	//---------------------------------------------------------------------------------
	for (iYear = nYear - 2; iYear > 0; iYear--)
	{
		//-----------------------------------------------------------------------------
