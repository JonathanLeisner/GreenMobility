...
//
//------------------------------------------------------------------------------
// Utilities for use in estimation module.
//------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
int unpackPars(double *param, double *beta, double *sigma, double *gamma, double *delta, double *xi, double *kappa, double *nu, double *lambda, double *phi)
{
    int iParam, iCnt;
    ...
    {
        beta[iParam] = param[iCnt];
        iCnt += 1;
    }

    for (iParam = 0; iParam < nChoices; iParam++)
    {
        sigma[iParam] = param[iCnt];
        iCnt += 1;
    }

    for (iParam = 0; iParam < nChoices; iParam++)
    {
        gamma[iParam] = param[iCnt];
        iCnt += 1;
    }

    for (iParam = 0; iParam < nChoices; iParam++)
    {
        delta[iParam] = param[iCnt];
        iCnt += 1;
    }

    for (iParam = 0; iParam < 2*nChoices; iParam++)
    {
    	xi[iParam] = param[iCnt];
        iCnt += 1;
    }

    for (iParam = 0; iParam < 4; iParam++)
    {
        kappa[iParam] = param[iCnt];
        iCnt += 1;
    }
    ...
    for (iParam = 0; iParam < nChoices; iParam++)
    {
    	lambda[iParam] = param[iCnt];
        iCnt += 1;
    }

    for (iParam = 0; iParam < 4; iParam++)
    {
    	phi[iParam] = param[iCnt];
        iCnt += 1;
    }
