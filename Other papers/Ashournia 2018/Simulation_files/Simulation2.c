...
int Simulation2()
{
	...
	//----------------------------------------------------------
	// Unemployment probability for manufacturing gets a shock
	//----------------------------------------------------------
	for (iYear = Yr_shock - FirstYr; iYear < LastYr_sim - FirstYr + 1; iYear++)
	{
		for (age = 30; age <= 65; age++)
		{
			for (female = 0; female < CatGender; female++)
			{
				...
				{
					for (lagSec = 0; lagSec <= nSector; lagSec++)
					{
						if (lagSec == 2)
						{
							Delta[iDelta] = fmin(Delta[iDelta] * 2.0, 1.0);
						}
					}
				}
			}
	...
	fp = fopen("Output/Simulation2.txt", "w");
