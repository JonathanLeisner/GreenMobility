...
int Simulation0()
{
	...
	while (RE_loop == 1)//(RE_loop<=2)
	{
		...
		// Simulation
		//------------------------------------------------------
		for (iYear = FirstYr_sim - FirstYr; iYear < LastYr_sim - FirstYr + 1; iYear++)
		{
			year = iYear + FirstYr;
			printf("Year: %d\n", year);
	...
	fp = fopen("Output/Simulation0.txt", "w");
	...
	fp = fopen("Output/Sim0Micro.txt", "w");
	fprintf(fp, "n\tYear\tCohort\tAge\tEduc\tElig\tFemale\tSector0\tLagSector0\tWage0\tWelfare0\tCohortWgt0\n");
	for (iYear = FirstYr_sim - FirstYr; iYear < LastYr_sim - FirstYr; iYear++)
	{
		year = iYear + FirstYr;
		if (year >= 2159 && year <= 2194)
		{
			for (iAge = 0; iAge < nAge; iAge++)
			{
				...
				iCoh = year - 1943 - age;
				coh = year - age;
				if (iCoh >= 151 && iCoh <= 186)
				{
					for (iN = 0; iN < nPop_sim; iN++)
					{
						fprintf(fp, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%lf\t%lf\t%lf\n", 
							iN+1, year, coh, age, EducSim[iSimPopCoh], EligSim[iSimPopCoh],
							FemaleSim[iSimPopCoh], ChoiceSim[iSimPopCohAge], LagSectorSim[iSimPopCohAge],
							WageSim[iSimPopCohAge], WelfareSim[iSimPopCohAge], CohortWgt[iCoh]);
					}
				}
			}
		}
