...
module initialize_data
	...
	! INDICES
	integer :: choice, lw, ed, xper, mar, i_g, t, n, i, d

	! DIMENSIONS 
	integer,parameter :: dt		= 30000		! Number of draws for expected values (i.e. for Monte Carlo integration).
...
!******************************************************************
	...
	do choice=0,1					! This loops over work or not.
		do xper=0,maxexp		! Even though experince is not in the NL income equation, we need to loop over it
			...
			do ed=0,2	
				do lw=0,1
					do mar=0,1
							nlincome(choice,stock(xper,ed,lw,mar),nt)= &
							exp(nli_intercept + (choice*nli_work) + nli_educ(ed) + (mar*nli_married))
					end do
				end do
			end do
		end do
		...
		do choice=0,1
			do xper=0,maxexp
				do ed=0,2
					do lw=0,1
						do mar=0,1
							v(choice,stock(xper,ed,lw,mar),nt)= &
							utfunc(choice,stock(xper,ed,lw,mar),nt)
						end do
					end do	
				end do
			end do
	...
	do choice=0,1					! This loops over work or not.
		do xper=0,maxexp		! Even though experince is not in the NL income equation, we need to loop over it
			...
			do ed=0,2	
				do lw=0,1
					do mar=0,1
							nlincome(choice,stock(xper,ed,lw,mar),nt-1)= &
							exp(nli_intercept + (choice*nli_work) + nli_educ(ed) + (mar*nli_married))
					end do
				end do
			end do
	...
	do d=1,dt,1		! Looping over the error draws.
		...
		do choice=0,1
			do xper=0,maxexp
				do ed=0,2
					do lw=0,1
						do mar=0,1
							v(choice,stock(xper,ed,lw,mar),nt-1)= &
							utfunc(choice,stock(xper,ed,lw,mar),nt-1) + &
							beta*Ev_no_max(stock(min(maxexp,xper+choice),ed,choice,mar),nt)
						end do
					end do	
				end do
			end do
		...
		do xper=0,maxexp
			do ed=0,2
				do lw=0,1
					do mar=0,1
						v_off_max(stock(xper,ed,lw,mar),nt-1)=max(v(0,stock(xper,ed,lw,mar),nt-1) &
																 ,v(1,stock(xper,ed,lw,mar),nt-1))
					end do
				end do
			end do
		end do
		...
		do choice=0,1					! This loops over work or not.
			do xper=0,maxexp		! Even though experince is not in the NL income equation, we need to loop over it
				...
				do ed=0,2	
					do lw=0,1
						do mar=0,1
								nlincome(choice,stock(xper,ed,lw,mar),t)= &
								exp(nli_intercept + (choice*nli_work) + nli_educ(ed) + (mar*nli_married))
						end do
					end do
				end do
		...
		do d=1,dt,1		! Looping over the error draws.
			...
			do xper=0,maxexp
				do ed=0,2
					do lw=0,1
						do mar=0,1
							v_off_max(stock(xper,ed,lw,mar),t)=max(v(0,stock(xper,ed,lw,mar),t) &
																  ,v(1,stock(xper,ed,lw,mar),t))
						end do
					end do
				end do
...
subroutine simulations
	...
	do t = 1,5
		do ed=0,2
			do xper=0,maxexp
				do lw=0,1
					do mar=0,1	
						!print*,t,ed,xper,lw,mar,Ev_no_max(stock(xper,ed,lw,mar),t),Ev_off_max(stock(xper,ed,lw,mar),t)
					end do
				end do
			end do
		end do
	...
	do n=1,nobs
		do t=1,ntdata,1
			...
			do choice=0,1
				sim_nlincome(choice,n,t)=exp(nli_intercept + (choice*nli_work) + nli_educ(educ(n)) &
											+ (nli_married*married(n,t))) 
			end do
			...
			if (t==nt) then
				do choice=0,1
					sim_v(choice,n,t)=sim_utfunc(choice,n,t)
				end do
			...
			if (t<nt) then
				...
				if (lastwork(n,t)==1) then		! If the person worked last period,
					...
					do choice=0,1
						if (sim_v(choice,n,t)==max(sim_v(0,n,t),sim_v(1,n,t))) then
							work(n,t)=choice
						end if
					end do
				end if
