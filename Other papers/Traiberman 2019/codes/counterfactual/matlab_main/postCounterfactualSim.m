figurePath = '/Users/sharont/Dropbox/DanishLEEDProject/structuralEstimation_Final/figures/';
outputPath = '/Users/sharont/Dropbox/DanishLEEDProject/structuralEstimation_Final/counterfactualOutputs/';
...
ar_pf = load('actualRun_pf');

newcode2disco;
L1SS_pf = ar_pf.L1SS_pf;
LU1SS_pf = ar_pf.L1SS_pf;
state = ar_pf.state;
numT = ar_pf.numT;
...
realWageT = bsxfun(@plus,ar_pf.wageT_pf',-log(ar_pf.PpathT_pf'));
realWageCF = bsxfun(@plus,cf_pf.wagecf_pf',-log(cf_pf.Ppathcf_pf'));

tic
[LPathT, LPathUT, occDistPathT, piPathT, piUPathT] = makeLPath(L1SS_pf,LU1SS_pf,realWageT',ar_pf.VPathT_pf,ar_pf.VPathUT_pf,state,n,b,un,params,TAMAT,TAMAT_U,cMat,hMat,ar_pf.numT);
toc
...
dlmwrite(strcat(outputPath,'historyT.csv'),[historyTMat wageTMat]);
dlmwrite(strcat(outputPath,'historyCF.csv'),[historyCFMat wageCFMat]);
...
meanManChange = mean((ar_pf.wageT_pf(1:13,1:capT) - cf_pf.wagecf_pf(1:13,1:capT))*100);
meanServChange = mean((ar_pf.wageT_pf(14:25,1:capT) - cf_pf.wagecf_pf(14:25,1:capT))*100);
meanFIREChange = mean((ar_pf.wageT_pf(26:32,1:capT) - cf_pf.wagecf_pf(26:32,1:capT))*100);
meanHEChange = mean((ar_pf.wageT_pf(33:38,1:capT) - cf_pf.wagecf_pf(33:38,1:capT))*100);
...
hgexport(fig1_rev,strcat(figurePath,'nominalSectorWageDelta'))

% 1. Changes in Skill Prices Over Time [Should we do a table?]
% 1a. Nominal
fig1 = figure;
hold on
nomServLines = plot(repmat((-1:capT-2)',1,25),(ar_pf.wageT_pf(14:end,1:capT)' - cf_pf.wagecf_pf(14:end,1:capT)')*100,'-','Color',[1 1 1]*.85,'LineWidth',.9);
nomManLines = plot(repmat((-1:capT-2)',1,13),(ar_pf.wageT_pf(1:13,1:capT)' - cf_pf.wagecf_pf(1:13,1:capT)')*100,'-','Color','blue','LineWidth',1.3);
...
hold off
hgexport(fig1,strcat(figurePath,'nominalWagesDelta'))
...
hold off
hgexport(fig2,strcat(figurePath,'realWagesDelta'))
...
for t = 1:numT
    gdpPathT_pf(t) = (dot(occDistPathT(:,t),exp(ar_pf.wageT_pf(:,t))) + ar_pf.pKT_pf(t)*KINC)/ar_pf.PpathT_pf(t);
...
hgexport(fig3,strcat(figurePath,'GDPpath'))
...
wageT_pf = ar_pf.wageT_pf;
PpathT_pf = ar_pf.PpathT_pf;
...
varDecompM = [levelMean_os levelMean_o levelMean_s;
diffSRmean_os diffSRmean_o diffSRmean_s;
diffLRmean_os diffLRmean_o diffLRmean_s]'*100;


dlmwrite(strcat(outputPath,'varDecomp.csv'),varDecompM);
