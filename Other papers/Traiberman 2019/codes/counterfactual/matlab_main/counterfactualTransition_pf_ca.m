...
delPFpath = cfPaths.simDELPF;
delZpath = cfPaths.simDELZ;

EXPSHIFTInit = EXPSHIFT_D(EXPSHIFT_D(:,1)==1,3);
...
% First Guess: Myopic Steady State
wPath0 = cfPaths.wagePathMyopia;
pKPath0 = cfPaths.pKPathMyopia;


% Preallocate some empty cells
numT = size(cfPaths.wagePathMyopia,2);
...
diary off
diary(strcat('cfTransitionLog',date,'_',num2str(hour(datetime('now'))),'.txt'))
diary on
...
save('counterfactualRun_pf')
save('rcf_pf','rcf_pf')
save('L1cf_pf','L1cf_pf');
save('LU1cf_pf','LU1cf_pf');
save('Pcf_pf','Pcf_pf');
save('pKcf_pf','pKcf_pf');
save('wagecf_pf','wagecf_pf');
save('Ppathcf_pf','Ppathcf_pf');
save('VPathcf_pf','VPathcf_pf');
save('VPathUcf_pf','VPathUcf_pf');
...
% for j = 1:38
% subplot(5,8,j)
% %plot(1:len,((wageT_pf(j,startDate:end) - log(PpathT_pf(startDate:end)) - wagecf_pf(j,startDate:end) + log(Ppathcf_pf(startDate:end)) - wageT_pf(j,startDate) + log(PpathT_pf(startDate)) + wagecf_pf(j,startDate) - log(Ppathcf_pf(startDate))))*100,1:len,zeros(1,len))
% plot(1:len,((wageT_pf(j,startDate:end) - log(PpathT_pf(startDate:end)) - wagecf_pf(j,startDate:end) + log(Ppathcf_pf(startDate:end))))*100,1:len,zeros(1,len))
% %ylim([-.25,.25])
% end
% set(gcf,'NextPlot','add');
% axes;
% h = title('Real Wage Differences Between Actual and Fixed Price Equilibria: Manufacturing');
% set(gca,'Visible','off');
% set(h,'Visible','on');
% 
% %%
...
% for j = 1:38
% subplot(8,5,j)
% plot((wageT_pf(j,startDate:end) - log(PpathT_pf(startDate:end)) - wagecf_pf(j,startDate:end) + log(Ppathcf_pf(startDate:end)))*100)
% %ylim([-1,1])
% end
% linkaxes
% set(gcf,'NextPlot','add');
...
% 
% %%
% spDiffs = (wageT_pf - repmat(log(PpathT_pf),38,1)) - (wagecf_pf - repmat(log(Ppathcf_pf),38,1));
% figure
% hold on
% ksdensity(spDiffs(:,9))
% ksdensity(spDiffs(:,14))
% ksdensity(spDiffs(:,18))
% ksdensity(spDiffs(:,40))
% hold off
% legend('+1','+5', '+10', 'SS')
% 
% figure
% hold on
% ksdensity(spDiffs(1:13,9))
% ksdensity(spDiffs(1:13,14))
% ksdensity(spDiffs(1:13,18))
% ksdensity(spDiffs(1:13,40))
% hold off
% legend('+1','+5', '+10', 'SS')
% 
% 
% %%
% 
% IY = state(:,4)<=13 & state(:,1)<=20;
% IO = state(:,4)<=13 & state(:,1)>=20;
% 
% close all
% figure
% hold on
% ksdensity((log(VPathT_pf(IY,10)) - log(VPathcf_pf(IY,10)))*100)
% ksdensity((log(VPathT_pf(IO,10)) - log(VPathcf_pf(IO,10)))*100)
% hold off
% legend('Young','Old')
% xlabel('Lifetime % Difference in Income')
% ylabel('Freq.')
% title('Heterogeneity in Lifetime Effects of Trade Shocks: Manufacturing - Short Run')


% figure
% hold on
% ksdensity(((VPathT_pf(IY,2)) - (VPathcf_pf(IY,2)))*55000)
% ksdensity(((VPathT_pf(IO,2)) - (VPathcf_pf(IO,2)))*55000)
% hold off
% legend('Young','Old')
% xlabel('Lifetime Earnings Difference in 2015 USD')
% ylabel('Freq.')
% title('Heterogeneity in Lifetime Effects of Trade Shocks: Manufacturing - Short Run')
% % 
% figure
% hold on
% ksdensity(((VPathT_pf(IY,40)) - (VPathcf_pf(IY,40)))*55000)
% ksdensity(((VPathT_pf(IO,40)) - (VPathcf_pf(IO,40)))*55000)
% hold off
% legend('Young','Old')
% xlabel('Lifetime Earnings Difference in 2015 USD')
% ylabel('Freq.')
% title('Heterogeneity in Lifetime Effects of Trade Shocks: Manufacturing - Long Run')
% 
% figure
% hold on
% ksdensity(((VPathT_pf(IY,40)) - (VPathT_pf(IY,10)))*55000)
% ksdensity(((VPathT_pf(IO,40)) - (VPathT_pf(IO,10)))*55000)
% hold off
% legend('Young','Old')
% xlabel('Lifetime Earnings Difference in 2015 USD')
% ylabel('Freq.')
% title('Heterogeneity in Lifetime Effects of Trade Shocks: Manufacturing - Short v Long Run')
% 
% figure
% hold on
% ksdensity(((VPathcf_pf(IY,40)) - (VPathcf_pf(IY,10)))*55000)
% ksdensity(((VPathcf_pf(IO,40)) - (VPathcf_pf(IO,10)))*55000)
% hold off
% legend('Young','Old')
% xlabel('Lifetime Earnings Difference in 2015 USD')
% ylabel('Freq.')
...
% figure
% hold on
% ksdensity((log(VPathT_pf(IY,end)) - log(VPathcf_pf(IY,end)))*100)
% ksdensity((log(VPathT_pf(IO,end)) - log(VPathcf_pf(IO,end)))*100)
% hold off
% legend('Young','Old')

% figure
% hold on
% ksdensity(((VPathT_pf(I,10))./(VPathcf_pf(I,10)))*100,'bandwidth',0.05)
% ksdensity(((VPathT_pf(I,end))./(VPathcf_pf(I,end)))*100,'bandwidth',0.05)
% hold off
% legend('+2','SS')