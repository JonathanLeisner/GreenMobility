...
time_pf = load('TIMING_counterfactualRun_pf');

newcode2disco;
L1SS_pf = time_pf.L1SS_pf;
LU1SS_pf = time_pf.L1SS_pf;
state = time_pf.state;
numT = cf_pf.numT;
...
%%%%%%%%%%%%%%%%%%%%%
close all
capT = 22; % number of periods to plot out

realWageT = bsxfun(@plus,time_pf.wagecf_pf',-log(time_pf.Ppathcf_pf'));
realWageCF = bsxfun(@plus,cf_pf.wagecf_pf',-log(cf_pf.Ppathcf_pf'));

tic
[LPathT, LPathUT, occDistPathT, piPathT, piUPathT] = makeLPath(L1SS_pf,LU1SS_pf,realWageT',time_pf.VPathcf_pf,time_pf.VPathUcf_pf,state,n,b,un,params,TAMAT,TAMAT_U,cMat,hMat,time_pf.numT);
toc
...
dlmwrite(strcat(outputPath,'historyT_TIMING.csv'),[historyTMat wageTMat]);
dlmwrite(strcat(outputPath,'historyCF_TIMING.csv'),[historyCFMat wageCFMat]);
...
meanManChange = mean((time_pf.wagecf_pf(1:13,1:capT) - cf_pf.wagecf_pf(1:13,1:capT))*100);
meanServChange = mean((time_pf.wagecf_pf(14:25,1:capT) - cf_pf.wagecf_pf(14:25,1:capT))*100);
meanFIREChange = mean((time_pf.wagecf_pf(26:32,1:capT) - cf_pf.wagecf_pf(26:32,1:capT))*100);
meanHEChange = mean((time_pf.wagecf_pf(33:38,1:capT) - cf_pf.wagecf_pf(33:38,1:capT))*100);
...
pbaspect([2 1 1])
hgexport(fig1_rev,strcat(figurePath,'nominalSectorWageDelta_TIMING'))

% 1. Changes in Skill Prices Over Time [Should we do a table?]
% 1a. Nominal
fig1 = figure;
hold on
nomServLines = plot(repmat((-1:capT-2)',1,25),(time_pf.wagecf_pf(14:end,1:capT)' - cf_pf.wagecf_pf(14:end,1:capT)')*100,'-','Color',[1 1 1]*.85,'LineWidth',.9);
nomManLines = plot(repmat((-1:capT-2)',1,13),(time_pf.wagecf_pf(1:13,1:capT)' - cf_pf.wagecf_pf(1:13,1:capT)')*100,'-','Color','blue','LineWidth',1.3);
...
hold off
hgexport(fig1,strcat(figurePath,'nominalWagesDelta_TIMING'))
...
hold off
hgexport(fig2,strcat(figurePath,'realWagesDelta_TIMING'))
...
for t = 1:numT
    gdpPathT_pf(t) = (dot(occDistPathT(:,t),exp(time_pf.wagecf_pf(:,t))) + time_pf.pKcf_pf(t)*KINC)/time_pf.Ppathcf_pf(t);
...
hgexport(fig3,strcat(figurePath,'GDPpath_TIMING'))
...
wageT_pf = time_pf.wagecf_pf;
PpathT_pf = time_pf.Ppathcf_pf;
...
hgexport(fig5,strcat(figurePath,'dynamicVarianceDecomposition_TIMING'))
...
diffSSRmean_os = VD2(2,2)./VD2(1,2);
diffSSRmean_o = VDocc(2,2)./VDocc(1,2);
diffSSRmean_s = VDsector(2,2)./VDsector(1,2);

diffSRmean_os = VD2(2,11)./VD2(1,11);
diffSRmean_o = VDocc(2,11)./VDocc(1,11);
diffSRmean_s = VDsector(2,11)./VDsector(1,11);

diffLRmean_os = VD2(2,end)./VD2(1,end);
diffLRmean_o = VDocc(2,end)./VDocc(1,end);
diffLRmean_s = VDsector(2,end)./VDsector(1,end);

varDecompM_TIMING = [levelMean_os levelMean_o levelMean_s;
diffSSRmean_os diffSSRmean_o diffSSRmean_s;
diffSRmean_os diffSRmean_o diffSRmean_s;
diffLRmean_os diffLRmean_o diffLRmean_s]'*100;


dlmwrite(strcat(outputPath,'varDecomp_TIMING.csv'),varDecompM_TIMING);
