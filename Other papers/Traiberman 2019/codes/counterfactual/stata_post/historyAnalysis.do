global postFiles /Users/sharont/Dropbox/DanishLEEDProject/structuralEstimation_Final/counterfactualOutputs/
global sandbox /Users/sharont/Dropbox/DanishLEEDProject/structuralEstimation_Final/counterfactualOutputs/
...
insheet using historyCF.csv, c clear
drop v3
...
rename v9 wageCF
compress
save historyCF, replace

insheet using historyT.csv, c clear
drop v3
rename v# v#,addnumber(1)
...
use historyT, clear
merge 1:1 pid t using historyCF
drop _merge

save mergedHistories, replace





use mergedHistories, clear
...
esttab diff1Dist fracDip1 using $tablePath/diff1Dist.tex,tex replace cells("mean(fmt(%4.2f)) sd(fmt(%4.2f) pattern(1 0)) p5(fmt(%4.2f) pattern(1 0)) p50(fmt(%4.2f) pattern(1 0)) p95(fmt(%4.2f) pattern(1 0))") varlabels(`e(labels)') fragment plain noobs non mlabels(none) collabels(none)
...
esttab diff2Dist fracDip2 using $tablePath/diff2Dist.tex,tex replace cells("mean(fmt(%4.2f)) sd(fmt(%4.2f) pattern(1 0)) p5(fmt(%4.2f) pattern(1 0)) p50(fmt(%4.2f) pattern(1 0)) p95(fmt(%4.2f) pattern(1 0))") varlabels(`e(labels)') fragment plain noobs non mlabels(none) collabels(none)
...
label define typeNames 1 "Short, Low AA", modify
label define typeNames 2 "Short, High AA", modify
label define typeNames 3 "Med, Low AA", modify
label define typeNames 4 "Med, High AA", modify
...
graph export $figurePath/cfOutcomesByAge.eps, replace
