...
insheet using historyCF_TIMING.csv, c clear
drop v3
...
rename v9 wageCF
compress
save historyCF_TIMING, replace

insheet using historyT_TIMING.csv, c clear
drop v3
rename v# v#,addnumber(1)
...
use historyT, clear
merge 1:1 pid t using historyCF_TIMING
drop _merge

save mergedHistories_TIMING, replace





use mergedHistories_TIMING, clear
...
esttab diff1Dist fracDip1 using $tablePath/diff1Dist_TIMING.tex,tex replace cells("mean(fmt(%4.2f)) sd(fmt(%4.2f) pattern(1 0)) p5(fmt(%4.2f) pattern(1 0)) p50(fmt(%4.2f) pattern(1 0)) p95(fmt(%4.2f) pattern(1 0))") varlabels(`e(labels)') fragment plain noobs non mlabels(none) collabels(none)
...
esttab diff2Dist fracDip2 using $tablePath/diff2Dist_TIMING.tex,tex replace cells("mean(fmt(%4.2f)) sd(fmt(%4.2f) pattern(1 0)) p5(fmt(%4.2f) pattern(1 0)) p50(fmt(%4.2f) pattern(1 0)) p95(fmt(%4.2f) pattern(1 0))") varlabels(`e(labels)') fragment plain noobs non mlabels(none) collabels(none)
...
graph export $figurePath/cfOutcomesByAge_TIMING.eps, replace
...
graph bar logDiff, over(age) by(type, note("Ages binned into 5 year groups.")) ytitle("Log Difference in Lifetime Earnings") scheme(s1mono)
graph export $figurePath/cfOutcomesByAge_TIMING_ANNOUNCE.eps, replace
